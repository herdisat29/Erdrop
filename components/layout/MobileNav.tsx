...
export function MobileNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 w-full z-40 flex items-center gap-2 overflow-x-auto px-4 pb-4 pt-2 bg-surface/90 backdrop-blur-md rounded-t-xl border-t-2 border-primary-container/20 scrollbar-hide">
      ...
      <Link href="/import" className={`flex flex-col items-center justify-center rounded-full squishy-interaction min-w-fit transition-colors ${
        pathname === '/import' 
          ? 'bg-primary-container text-on-primary-container px-6 py-1.5' 
          : 'text-on-surface-variant px-4 py-2 hover:bg-surface-container-highest'
      }`}>
        <span className="material-symbols-outlined" style={pathname === '/import' ? {fontVariationSettings: "'FILL' 1"} : {}}>upload_file</span>
      ...
      <Link href="/logs" className={`flex flex-col items-center justify-center rounded-full squishy-interaction min-w-fit transition-colors ${
        pathname === '/logs' 
          ? 'bg-primary-container text-on-primary-container px-6 py-1.5' 
          : 'text-on-surface-variant px-4 py-2 hover:bg-surface-container-highest'
      }`}>
        <span className="material-symbols-outlined" style={pathname === '/logs' ? {fontVariationSettings: "'FILL' 1"} : {}}>assignment</span>
